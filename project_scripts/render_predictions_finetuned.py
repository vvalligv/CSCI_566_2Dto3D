...
Renders each predicted mesh from the depth-supervised finetuned model
from all 8 camera poses. Saves individual views and a contact sheet per object.

Output:
  prediction_renders_depth/{hard,easy}/{uid}_camview{i}.png
  prediction_renders_depth/{hard,easy}/{uid}_sheet.png
  gt_renders/{hard,easy}/{uid}_camview{i}.png      (shared with baseline)
...
PRED_OUT_DIR = os.path.join(SCRIPT_DIR, "prediction_renders_depth")
GT_OUT_DIR   = os.path.join(SCRIPT_DIR, "gt_renders")   # shared with baseline
...
def main():
    ...
    for uid, info in inf_manifest.items():
        label     = info["label"]
        mesh_path = info["mesh_path"]   # single mesh per object
        ...
        # ── Render GT (reuse if already generated by baseline step) ──────────
        ...
        # ── Render finetuned predicted mesh ───────────────────────────────────
        if not os.path.exists(mesh_path):
            print(f"  pred SKIP: {mesh_path} not found")
            continue

        sheet_path = os.path.join(pred_out, f"{uid}_sheet.png")
        ...
        try:
            ...
            renders   = render_views(pred_mesh, pred_out, uid)
            if renders:
                make_contact_sheet(renders, sheet_path)
                print(f"  pred → {len(renders)} renders + sheet")
        except Exception as e:
            print(f"  pred ERROR: {e}")

    print(f"\nDone.")
    print(f"  Finetuned renders → {PRED_OUT_DIR}")
