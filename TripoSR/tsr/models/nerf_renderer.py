...
class TriplaneNeRFRenderer(BaseModule):
    ...
    def _forward(
        ...
        **kwargs,
        ...
        # Depth: expected z value along ray
        comp_depth_ = (weights * z_vals).sum(dim=-1)  # (N_rays,)
        ...
        comp_depth = torch.zeros(n_rays, dtype=comp_depth_.dtype, device=comp_depth_.device)
        comp_rgb[rays_valid] = comp_rgb_
        opacity[rays_valid] = opacity_
        comp_depth[rays_valid] = comp_depth_

        comp_rgb += 1 - opacity[..., None]
        comp_rgb = comp_rgb.view(*rays_shape, 3)
        comp_depth = comp_depth.view(*rays_shape)

        return comp_rgb, comp_depth
    ...
    def forward(
        ...
        if triplane.ndim == 4:
            comp_rgb, comp_depth = self._forward(decoder, triplane, rays_o, rays_d)
        else:
            results = [
                self._forward(decoder, triplane[i], rays_o[i], rays_d[i])
                for i in range(triplane.shape[0])
            ]
            comp_rgb = torch.stack([r[0] for r in results], dim=0)
            comp_depth = torch.stack([r[1] for r in results], dim=0)
